...
func main() {
	...
	logCounter++
	log.Info("incremented the counter to: " + string(logCounter))
...
func send(logStruct LogPayload) {
	log.Info("sending payload")
	log.Info(logStruct.Logs)
	arr, err := json.Marshal(logStruct)
	if err != nil {
		log.Error(err)
	} else {
	...
	if httpErr != nil {
		...
		log.Info("HTTP response code: " + string(resp.StatusCode))
	}
}
